...
| `profile-discovery-service.js` | TikTok profile discovery: scrapes a user's profile page for video posts, deduplicates against existing Jobs and DiscoveredPosts, creates DiscoveredPost documents, downloads thumbnails + avatar. Also provides oEmbed-based thumbnail repair via `repairThumbnailsViaOembed`. **Exception to statelessness rule**: imports `Job` model for deduplication queries (bounded `$in` lookups) and `DiscoveredPost` model for persistence. Exports `{ triggerProfileDiscovery, scrapeProfileVideos, extractHandleFromTikTokUrl, normalizeHandle, resolveDiscoveryHandle, repairThumbnailsViaOembed }`. ~670 lines. Stat extraction: Strategies 1 & 2 use `parseStatNumber()` (handles "1.2M"/"45.3K"/raw numbers) and `extractCardStats()` (20+ selector fallbacks for data-e2e and class patterns) to extract playCount, diggCount, shareCount, commentCount from DOM. Stats are merged with Math.max so JSON stats (Strategy 3/4) win when both sources have values. |
